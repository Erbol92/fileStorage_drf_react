...
class FileViewSet(viewsets.ModelViewSet):
    ...
    def create_folder(self, request):
        ...
        if parent_id:
            ...
            parent = get_object_or_404(
                File, parent=parent_id, owner=request.user)
    ...
    def move(self, request, pk=None):
        ...
        new_parent = get_object_or_404(
            File, id=new_parent_id, owner=item.owner)
        ...
        # item.save()
        new_relative_path = user_directory_path(
            item, os.path.basename(item.file.name))
    ...
    def rename(self, request, pk=None):
        ...
        old_physical_path = item.file.path
        new_relative_path = user_directory_path(
            item, os.path.basename(new_name))
        new_physical_path = os.path.join(MEDIA_ROOT, new_relative_path)
        try:
            if os.path.exists(old_physical_path):
                os.rename(old_physical_path, new_physical_path)
